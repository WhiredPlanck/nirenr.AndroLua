...
//
//#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "lua", __VA_ARGS__)

static void ternary_ret(LexState *ls, expdesc *e) {
    /*
     * a=b?c:d
     * =>
     * a=(function(b) if b then return c end return d end)(b)
     */
    int line = ls->linenumber;
    expdesc a = clone(*e);
    luaK_exp2nextreg(ls->fs, &a);
    if (e->k == VCALL) {
        luaK_setoneret(ls->fs, e);
    } else {
        luaK_storevar(ls->fs, &a, e);
    }
     ...
     * RET C
     */
    new_localvar(ls, NULL);//space for a

    adjustlocalvars(ls, 1);
    ls->fs->f->numparams = cast_byte(ls->fs->nactvar);
    luaK_reserveregs(ls->fs, ls->fs->nactvar);  /* reserve register for parameters */
    ...
    luaK_codeABC(ls->fs, OP_TEST, 0, 0, 0);//ra must be 0
    int jmp = luaK_jump(ls->fs);
    expr(ls, &b);
    int RB = luaK_exp2RK(ls->fs, &b);
    luaK_ret(ls->fs, RB, 1);
    luaK_patchtohere(ls->fs, jmp);
    luaK_reserveregs(ls->fs, -1);
    if (ls->t.token == TK_ELSE || ls->t.token == '`') {
        luaX_next(ls);
        expr(ls, &c);
        int RC = luaK_exp2RK(ls->fs, &c);
        luaK_ret(ls->fs, RC, 1);
    } else {
        luaX_syntaxerror(ls, "missing 'else' or '`' in ternary operator");
    }
    lua_assert(ls->fs->f->maxstacksize >= ls->fs->freereg &&
               ls->fs->freereg >= ls->fs->nactvar);
    ls->fs->freereg = ls->fs->nactvar;  /* free registers */
    ...
    luaK_reserveregs(ls->fs, 1);
    luaK_exp2nextreg(ls->fs, &a);
    luaK_codeABC(ls->fs, OP_CALL, ls->fs->freereg - 2, 2, 2);
...
//foreah是为了for k,v : T do end代替for k,v in pairs(T) do end而存在的，所以动态push进独立的pairs就好
...
static void single_test_then_block(LexState *ls, int *escapelist) {
    ...
    if (ls->t.token == TK_ELSE || ls->t.token == TK_CASE ||
        ls->t.token == '`')  /* followed by 'else'/'elseif'? */
        luaK_concat(fs, escapelist, luaK_jump(fs));  /* must jump over it */
    luaK_patchtohere(fs, jf);
}
...
static void exprstat(LexState *ls) {
    ...
    struct LHS_assign v;
    suffixedexp(ls, &v.v);
    if (ls->t.token >= TK_ADDA && ls->t.token <= TK_SHRA) {
        int op = ls->t.token - TK_ADDA;
        luaX_next(ls);
        expdesc e;
        expr(ls, &e);
        expdesc a = clone(v.v);
        codebinexpval(fs, cast(OpCode, op + OP_ADD), &a, &e, ls->linenumber);
        luaK_storevar(fs, &v.v, &a);
    } else if (ls->t.token == TK_CONCATA) {
        luaX_next(ls);
        expdesc e;
        expr(ls, &e);
        expdesc a = clone(v.v);
        if (e.k == VCALL) {//A..=B(?)
            luaK_setoneret(fs, &e);  /* set call to return 1 result */
            if (a.k == VUPVAL || a.u.info + 1 != e.u.info) {// v.v鍜宔涓嶇浉杩炴垨鑰匲PVAL锛堝悗璁惧€硷級鎯呭喌
                luaK_infix(fs, OPR_CONCAT, &a);
                luaK_reserveregs(fs, 1);
                luaK_infix(fs, OPR_CONCAT, &e);
                luaK_posfix(fs, OPR_CONCAT, &a, &e, ls->linenumber);
                luaK_reserveregs(fs, -1);
                luaK_storevar(fs, &v.v, &a);
            } else {// v.v鍜宔涓€瀹氱浉杩�
                luaK_posfix(fs, OPR_CONCAT, &a, &e, ls->linenumber);
...
static void statement(LexState *ls) {
    ...
    switch (ls->t.token) {
        ...
        case TK_WHEN: {  /* stat -> whenstat */
            whenstat(ls, line, NULL);
            break;
        }
