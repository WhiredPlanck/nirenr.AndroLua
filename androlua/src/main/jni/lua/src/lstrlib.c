...
static int string_metatable_index(lua_State *L) {
    lua_settop(L, 2);
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, "string_metatable_index");
    lua_pushvalue(L, 2);
    lua_gettable(L, -2);
    lua_remove(L, -2);
    return 1;
...
//    lua_getfield(L,-1,"__index");
    lua_setfield(L, LUA_REGISTRYINDEX, "string_metatable_index");
    lua_pushcfunction(L, string_metatable_index);
