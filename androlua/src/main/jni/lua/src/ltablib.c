...
//mod by nirenr
#if defined(LUA_COMPAT_TABLIBADD)

static int add(lua_State *L) {
    lua_Integer pos = aux_getn(L, 1, TAB_RW) + 1;  /* first empty element */
    lua_pushnil(L);  /* first key */
    while (lua_next(L, 1)) {
        if (lua_rawequal(L, -1, 2)) {
            lua_pop(L, 1);
...
//mod by nwdxlgzs
#if defined(LUA_COMPAT_TABLIBFILL)
///*
// * fill(T,pos,count,v)
// */
//static int fill(lua_State *L) {
//    if(lua_gettop(L) == 3){ //fill(T,n,v)->fill(T,1,n,v)
//        lua_pushinteger(L, 1);
//        lua_insert(L, 2);
//    }
