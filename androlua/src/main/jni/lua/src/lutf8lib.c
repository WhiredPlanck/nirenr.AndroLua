...
static int Lutf8_lower(lua_State *L) { return convert(L, utf8_tolower); }

static int Lutf8_upper(lua_State *L) { return convert(L, utf8_toupper); }

static int Lutf8_title(lua_State *L) { return convert(L, utf8_totitle); }

static int Lutf8_fold(lua_State *L) { return convert(L, utf8_tofold); }
...
static int singlematch(MatchState *ms, const char *s, const char *p,
    ...
    if (s >= ms->src_end)
        ...
        switch (pch) {
            ...
            case L_ESC:
                utf8_decode(p, ms->p_end, &pch);
                return match_class(ch, pch);
            case '[':
                return matchbracketclass(ch, p - 1, ep - 1);
            default:
                return pch == ch;
        }
    }
...
static void add_s(MatchState *ms, luaL_Buffer *b, const char *s,
    ...
    while (news < new_end) {
        ...
        if (ch != L_ESC)
            ...
            if (!utf8_isdigit(ch)) {
                if (ch != L_ESC)
                    luaL_error(ms->L, "invalid use of "
                LUA_QL("%c")
